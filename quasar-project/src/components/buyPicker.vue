        ...
        <q-dialog v-model="mustPopWarning" v-if="event_type == optionsReBuyType[0]">
          <q-card>
            <div class="q-ma-xl col flex flex-center">
              ...
              <q-btn label="Annuler" @click="[mustPopWarning=false,rachatVal=DEFAULT_RACHAT_VAL_VALUE,listToDisplay=[]]"></q-btn>
              </div>
            </div>
          </q-card>
    </q-dialog>
...
const sendRebuyPicked = function (force=false) {
  ...
  else
  {
    ...
    {
      event_year_str=rachatVal.value.split('-')[1].split(' ')[0];
      event_month_str=rachatVal.value.split('-')[0].split(' ')[1];
