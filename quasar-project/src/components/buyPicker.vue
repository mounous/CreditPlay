<template>
  ...
  <div>
    ...
    <div class="row">
        ...
        ></q-select>
        <q-input v-if="event_type == optionsReBuyType[1]"  class="q-ma-md" dense style="max-width: 100px;" label="Date de rachar" bg-color="blue-grey-8" filled v-model="date_reloan" mask="date">
          ...
          <q-input v-if="event_type == optionsReBuyType[1]" class="q-ma-md" label="Nouveau taux" style="max-width:110px" maxlength="8" v-model="rate_reloan"
...
import {returnBaseData} from '../pages/credit_utility'
var reloanMax=ref(returnBaseData().end_year.toString()+'/'+returnBaseData().end_month.toString());
var reloanMin=ref(simu.value.credit.startingDate);
var date_reloan=ref('');
...
const sendRebuyPicked = function () {
  emit('update-from-rebuy-pick', { rebuyType:event_type.value,rebuyVal:rachatVal.value,reLoanDate:date_reloan.value,reloadRate:Number(rate_reloan.value),rebuyPenalties:Number(penalties_rebuy.value)});
};
//
