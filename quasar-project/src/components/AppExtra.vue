<template>
    <div style="display:flex;flex-direction: column;align-items: center;align-content: center;justify-content: center;justify-items: center;width: 100%;height: 100%;" >
        <transition name="bounce">
          <th class="q-ma-md text-center" style="color: white;font-size:20px;" v-if="initPhase>=0">{{transStr(stringsIDs.str_tuto_process_7)}}</th>
        </transition>
        <transition name="enter">
          <th class="q-ma-md text-center" style="color: white;font-size:20px;" v-if="initPhase>=1">{{transStr(stringsIDs.str_tuto_process_8)}}</th>
        </transition>
    </div>
</template>
...
const emit=defineEmits(['tuto-extra-finished']);
//var myscrolling=ref();
var initPhase=ref(1);
...
{
  emit('tuto-extra-finished');
}
const play=function()
{
  initPhase.value++;
  if(initPhase.value==2)
  {
